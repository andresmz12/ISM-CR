...
export default function ClientDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <Link to="/clients" className="text-sm text-indigo-600 hover:underline">&larr; Volver a clientes</Link>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              {client.source && <span className="text-xs text-gray-400">Origen: {client.source}</span>}
              {client.tags?.map((t) => (
          ...
          <div className="text-right text-sm text-gray-500">
            <div>Agente: <span className="font-medium text-gray-700">{client.assignedAgent?.fullName ?? 'Sin asignar'}</span></div>
            ...
            {canReassign && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <button onClick={handleReassign} disabled={!reassignTo}
                  className="rounded-md bg-gray-800 px-2 py-1 text-xs font-medium text-white disabled:opacity-40">
                  Asignar
                </button>
              </div>
            )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <h2 className="mb-3 text-sm font-semibold text-gray-900">Registrar interacción</h2>
          ...
          <form onSubmit={handleLogInteraction} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Nuevo estatus</label>
            ...
            <div>
              ...
              <textarea required rows={3} value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" placeholder="¿Qué se habló con el cliente?" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Próximo seguimiento</label>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <h2 className="mb-3 text-sm font-semibold text-gray-900">Historial de interacciones</h2>
          <div className="max-h-96 space-y-3 overflow-y-auto">
            {client.interactions.map((i) => (
              <div key={i.id} className="border-b border-gray-100 pb-2 text-sm last:border-0">
                <div className="flex items-center justify-between">
                  <span className="font-medium text-gray-800">{i.type}</span>
            ...
            {client.interactions.length === 0 && <p className="text-sm text-gray-400">Sin interacciones registradas.</p>}
          </div>
        </div>
      </div>
    </div>
