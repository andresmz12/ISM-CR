...
export default function ProjectsPage() {
  ...
  const fetchProjects = useCallback(() => {
    setLoading(true);
    return api.get('/projects').then((res) => setProjects(res.data)).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-5">
        ...
        {projects.map((p) => (
          ...
          >
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <span className="text-xs font-medium text-slate-400">{p._count.tasks} tareas</span>
            </div>
          </Link>
        ))}
        {projects.length === 0 && (
      ...
      {showModal && (
        <NewProjectModal onClose={() => setShowModal(false)} onCreated={() => { setShowModal(false); fetchProjects(); }} />
      )}
    </div>
  );
}
