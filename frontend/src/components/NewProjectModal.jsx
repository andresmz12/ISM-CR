      ...
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input required value={form.name} onChange={(e) => update('name', e.target.value)} className={inputCls} placeholder="Ej. Paquetería App" />
          </div>
          <div>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Repositorio (opcional)</label>
            <input value={form.repoUrl} onChange={(e) => update('repoUrl', e.target.value)} className={inputCls} placeholder="https://github.com/..." />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Equipo de la empresa</label>
            ...
            <p className="mt-1 text-xs text-slate-400">Solo estas personas verán esta empresa (además de Admin/Supervisor).</p>
          </div>
